...
const upsertGodown = async (
  isEditing: boolean,
  godownId: string | undefined,
  data: Record<string, unknown>,
): Promise<string> => {
  if (isEditing) {
    const { error } = await supabase.from('warehouses').update(data).eq('id', godownId);
...
export default function AddGodownScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Error', getErrorMessage(err, 'Failed to save godown'));
    } finally {
      setLoading(false);
    }
  };

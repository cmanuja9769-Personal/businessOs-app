...
import type { ISupplier, IItem, IPurchaseItem, IPurchase } from "@/types"
...
export function PurchaseEditor({ purchase }: PurchaseEditorProps) {
  ...
  const [purchaseDate, setPurchaseDate] = useState(new Date(purchase.date).toISOString().split("T")[0])
  const [gstEnabled, setGstEnabled] = useState(purchase.gstEnabled)
  const [purchaseItems, setPurchaseItems] = useState<IPurchaseItem[]>(purchase.items)
  const [notes, setNotes] = useState(purchase.notes || "")

  useEffect(() => {
    async function loadData() {
      try {
        const [suppliersData, itemsData] = await Promise.all([getSuppliers(), getItems()])
  ...
  const handleSave = async () => {
    ...
    try {
      const purchaseData = {
        ...
        balance: totals.total - purchase.paidAmount,
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input value={purchase.purchaseNo} disabled className="font-mono" />
            </div>
            <div className="space-y-2">
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={() => router.push(`/purchases/${purchase.id}`)} disabled={isSaving}>
          Cancel
        </Button>
